...
// Define the Hotkey function
void ImGui::Hotkey(int* k, const ImVec2& size_arg = ImVec2(0, 0))
{
    ...
    if (!waitingforkey)
    {
        // Display the button with the current key name
        if (ImGui::Button(KeyNames[*k], size_arg))
        {
            // Set waitingforkey to true when the button is clicked
            waitingforkey = true;
            key_pressed = false;
        }
    }
    ...
    {
        // Display the button with "..." to indicate waiting for a key press
        ...
        for (auto& Key : KeyCodes)
        {
            ...
            {
                // Debounce: Only register the key press if the previous state was not pressed
                if (!key_pressed)
                {
                    *k = Key;
                    waitingforkey = false;
            ...
            {
                // Reset key_pressed if no key is pressed
                key_pressed = false;
            }
        }
    }
